...
# All strictly-paired relationships.
sameaset = set()
...
def train():
    ...
    for e in tbar:
        ...
        ir_list, vi_list = gen_groups(args.trainNumber * re_times, sameaset)
        ir_paths.extend(ir_list)
        vi_paths.extend(vi_list)
        print(len(ir_paths), ir_paths[0])
        print(len(vi_paths), vi_paths[0])

        model.train()
        count = 0
        for batch in range(batches*(re_times)):
            optimizer.zero_grad()
            image_paths = ir_paths[batch * batch_size:(batch * batch_size + batch_size)]  # 取每个批次训练的图像
            image_paths_vis = vi_paths[batch * batch_size:(batch * batch_size + batch_size)]  # 取每个批次训练的图像
            # load image patches of this batch.
            image_ir = utils_ablation.get_train_images_auto(patchPrePath + "/ir", image_paths, mode="L");
            image_vi = utils_ablation.get_train_images_auto(patchPrePath + "/vis", image_paths_vis, mode="L");
            ...
            all_intensity_loss += loss_illu.item();
